...
#### Week 2.5: 단순화된 한국 상품 API 🆕
- [ ] 네이버 쇼핑 API 키 발급 및 설정
- [ ] 네이버 쇼핑 클라이언트 단독 구현 (MCP 없이)
- [ ] AI + 네이버 API 직접 연동 (단순 파이프라인)
- [ ] 실제 한국 상품 이미지 및 가격 정보 연동
- [ ] 성능 최적화 (3-5초 목표)
- [ ] 단순화된 테스트 및 검증

**📝 아키텍처 결정**: MVP 단순성을 위해 **MCP 파이프라인 생략**, 네이버 API 직접 사용
...
**진행률**: ✅ 12/12 (100%) **기본 MCP 완료** + 🆕 6개 확장 작업
...
*Next Review: Week 2 MCP 파이프라인 통합*